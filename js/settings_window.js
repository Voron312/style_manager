var style_manager_settings = function (cat_id, act_tab) {
  Drupal.css_gen.style_manager_settings_open_groups = {};
  Drupal.css_gen.style_manager_settings_open_groups[Drupal.css_gen.selectrd_group] = true;
  if (!act_tab) {
    act_tab = 0;
  }
  if (Drupal.css_gen.settings_win) {
    Drupal.css_gen.settings_win.destroy();
  }
  if (Drupal.css_gen.settings_group_win) {
    Drupal.css_gen.settings_group_win.destroy();
  }
  ...
  var updInfo = function (cat_id) {
    loadMask.show();
    Drupal.css_gen.settings_win_store.load();
  ...
  Drupal.css_gen.settings_win_form = Ext.create('Ext.form.Panel', {
    frame: true,
    bodyPadding: 5,
    width: 985,
    height: 440,
        ...
        handler: function () {
          Ext.MessageBox.confirm('Confirm', 'Are you sure?', function (btn) {
            ...
            jQuery.post(Drupal.settings.basePath + 'style_manager/category_destroy', data, function (data) {
              Drupal.css_gen.filter_category_tree_load();
              Drupal.css_gen.upd_page_styles('global_config');
              loadMask.hide();
              Drupal.css_gen.settings_win.destroy();
            }, 'json');
          });
        ...
        handler: function () {
          Ext.MessageBox.confirm('Confirm', 'Are you sure?', function (btn) {
            ...
            jQuery.post(Drupal.settings.basePath + 'style_manager/category_reset', data, function (data) {
              Drupal.css_gen.filter_category_tree_load(data.id);
              loadMask.hide();
              Drupal.css_gen.upd_page_styles('global_config')
              updInfo(data.id);
            }, 'json');
  ...
  Drupal.css_gen.export_import_form = Ext.create('Ext.form.Panel', {
    frame: true,
          ...
          if(form.isValid()){
            ...
            form.submit({
              ...
              success: function(fp, o) {
                ...
                Drupal.css_gen.filter_category_tree_load(o.result.id);
  ...
  var groupcollapseUpd = function () {
    if (Drupal.css_gen.style_manager_settings_open_groups != false) {
      return false;
    }
    Drupal.css_gen.style_manager_settings_open_groups = {};
    jQuery('.x-grid-group-hd').not('.x-grid-group-hd-collapsed').find('.x-grid-group-title').each(function () {
      Drupal.css_gen.style_manager_settings_open_groups[jQuery(this).html()] = true
    });
  };
  Drupal.css_gen.settings_win_store = Ext.create('Ext.data.Store', {
    autoLoad: false,
    autoSync: true,
    ...
    proxy: {
      ...
      api: {
        ...
        create: Drupal.settings.basePath + 'style_manager/category_form_item/save/' + cat_id + '/' + Drupal.css_gen.selected_cat,
    ...
    listeners: {
      ...
      load: function () {
        var open_groups = Drupal.css_gen.style_manager_settings_open_groups;
        Drupal.css_gen.style_manager_settings_open_groups = false;
  ...
  Drupal.css_gen.settings_win_grid = Ext.create('Ext.grid.Panel', {
    plugins: [rowEditing],
    features: [groupingFeature],
    width: 985,
    height: 440,
    store: Drupal.css_gen.settings_win_store,
    columns: [
      {
        text: 'ID',
        width: 120,
      ...
      },
      {
        ...
        field: {
          ...
          listConfig: {
            getInnerTpl: function() { return '<div  data-qtip="{description}" style="white-space: nowrap;">{name}</div>';}
          },
          listeners: {
            afterrender: typeUpd,
            change: typeUpd
        ...
        items: [
          {
            ...
            handler: function () {
              var fields = {};
              if (Drupal.css_gen.settings_win_grid.getView().getSelectionModel().getSelection()[0]) {
                var selected = Drupal.css_gen.settings_win_grid.getView().getSelectionModel().getSelection()[0]['data'];
                fields.group = selected.group;
                fields.selector = selected.selector;
              }
              Drupal.css_gen.settings_win_store.insert(0, new settings_model(fields));
          ...
          '-',
          {
            ...
            handler: function () {
              Ext.MessageBox.confirm('Confirm', 'Are you sure?', function (btn) {
                ...
                var selection = Drupal.css_gen.settings_win_grid.getView().getSelectionModel().getSelection()[0];
                if (selection) {
                  Drupal.css_gen.settings_win_store.remove(selection);
                }
              });
            }
          },
  ...
  Drupal.css_gen.settings_win = Ext.create('Ext.window.Window', {
    ...
    listeners: {
      beforedestroy: function () {
        if (Drupal.css_gen.selected_cat_arr && Drupal.css_gen.selected_cat_arr[1]) {
          Drupal.css_gen.styleStore.load({url: Drupal.css_gen.styleStoreProxy.url + '?cat=' + Drupal.css_gen.selected_cat});
        }
        Drupal.css_gen.upd_page_styles();
      }
    },
    items: [
      {
        ...
        items: [
          {
            title: 'Settings',
            items: Drupal.css_gen.settings_win_form
          },
          {
            title: 'Style form',
            id: 'tab_style_form',
            disabled: !(cat_id),
            items: Drupal.css_gen.settings_win_grid
          },
          {
            title: 'Import / export',
            id: 'tab_import_export',
            disabled: !(cat_id),
            items: Drupal.css_gen.export_import_form
          }
        ]
      }
    ]
  });
  Drupal.css_gen.settings_win.show();
